...
class PokerRoom(roomId: String, actorSystem: ActorSystem) {
  ...
  def websocketFlow(user: String,
                    isSpectator: Boolean): Flow[Message, Message, _] = {
    ...
    Flow
      .fromGraph(GraphDSL.create(source) { implicit builder =>
        {
          responseSource =>
            ...
            val pokerRoomActorSink =
              Sink.actorRef[PokerEvent](pokerRoomActor, UserLeft(user))

            val merge = builder.add(Merge[PokerEvent](2))

            val actorConnected =
              Flow[ActorRef].map(UserJoined(user, _, isSpectator))
      ...
      .keepAlive(FiniteDuration(5, TimeUnit.SECONDS),
                 () => TextMessage(toJson(HeartBeat())))
  }
  ...
  private def mapToIncomingPokerEvent(user: String,
    ...
    incomingMessage.get("eventType") match {
      ...
      case Some("showResult") =>
        fromJson[RequestShowEstimationResult](textContent)
          .copy(userName = user)
  ...
  private def mapToOutgoingTextMessage(pokerEvent: PokerEvent): TextMessage = {
    pokerEvent match {
      ...
      case RequestStartEstimation(name, taskName, startDate, _) =>
        TextMessage(toJson(pokerEvent.asInstanceOf[RequestStartEstimation]))

      case UserHasEstimated(name, taskName, _) =>
        TextMessage(toJson(pokerEvent.asInstanceOf[UserHasEstimated]))

      case EstimationResult(taskName, startDate, endDate, estimates, _) =>
...
object PokerRoom {
  def apply(roomId: String)(implicit actorSystem: ActorSystem) =
    new PokerRoom(roomId, actorSystem)
}
